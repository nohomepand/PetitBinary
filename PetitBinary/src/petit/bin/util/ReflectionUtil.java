...
//	/**
//	 * 対象のクラスの継承関係を親クラスの方へ向けて辿りつつ，全てのクラスの {@link Class#getDeclaredConstructors()} を走査する
//	 * 
//	 * @param begin 対象のクラス
//	 * @param visi 可視性の制約
//	 * @param walker コールバックメソッド
//	 */
//	public static final void visitConstructor(final Class<?> begin, final VisibilityConstraint visi, final WalkClassInheritanceTreeCallBack<Constructor<?>> walker) {
//		for (Class<?> cur = begin; cur != null; cur = cur.getSuperclass())
//			for (final Constructor<?> ctor : cur.getDeclaredConstructors()) {
//				if (visi.isVisible(begin, ctor))
//					if (!walker.visit(ctor))
//						return;
//			}
//	}
...
//		for (Class<?> cur = begin; cur != null; cur = cur.getSuperclass()) {
//			for (Class<?> enc = cur; enc != null; enc = enc.getEnclosingClass()) {
//				for (final Field field : enc.getDeclaredFields()) {
...
//		for (Class<?> cur = begin; cur != null; cur = cur.getSuperclass()) {
//			for (Class<?> enc = cur; enc != null; enc = enc.getEnclosingClass()) {
//				for (final Method method : enc.getDeclaredMethods()) {
...
////		visitField(BaseStruct.SubA.class, VisibilityConstraint.PUBLIC_VIEWPOINT, new WalkClassInheritanceTreeCallBack<Field>() {
////			@Override
////			public boolean visit(Field ao) {
...
////			}
////		});
//		Iterable<Field> a = getVisibleFields(BaseStruct.SubA.class, VisibilityConstraint.INHERITED_CLASS_VIEWPOINT, null, long.class);
//		System.out.println(a);
//		List<Method> b = getVisibleMethods(Hoge.class, VisibilityConstraint.INHERITED_CLASS_VIEWPOINT, null, null, BinaryInput.class, Object.class, Field.class);
////		List<Method> b = getVisibleMethods(Hoge.class, VisibilityConstraint.INHERITED_CLASS_VIEWPOINT, null, null, (Class<?>[]) null);
